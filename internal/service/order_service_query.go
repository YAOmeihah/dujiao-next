...
func (s *OrderService) GetOrderByGuestForTenant(tenant TenantContext, orderID uint, phone, password string) (*models.Order, error) {
	phone = canonicalizeGuestPhone(phone)
	order, err := s.orderRepo.GetByIDAndGuestScoped(orderID, phone, password, orderScopeFromTenant(tenant))
	if err != nil {
		return nil, ErrOrderFetchFailed
	}
	if order == nil {
...
func (s *OrderService) GetOrderByGuestOrderNoForTenant(tenant TenantContext, orderNo, phone, password string) (*models.Order, error) {
	phone = canonicalizeGuestPhone(phone)
	order, err := s.orderRepo.GetByOrderNoAndGuestScoped(orderNo, phone, password, orderScopeFromTenant(tenant))
	if err != nil {
		return nil, ErrOrderFetchFailed
	}
	if order == nil {
...
func (s *OrderService) GetAnyOrderByGuestOrderNoForTenant(tenant TenantContext, orderNo, phone, password string) (*models.Order, error) {
	phone = canonicalizeGuestPhone(phone)
	order, err := s.orderRepo.GetAnyByOrderNoAndGuestScoped(orderNo, phone, password, orderScopeFromTenant(tenant))
	if err != nil {
		return nil, ErrOrderFetchFailed
	}
	if order == nil {
...
func (s *OrderService) ListOrdersByGuestForTenant(tenant TenantContext, phone, password string, page, pageSize int) ([]models.Order, int64, error) {
	phone = canonicalizeGuestPhone(phone)
	orders, total, err := s.orderRepo.ListByGuestScoped(phone, password, page, pageSize, orderScopeFromTenant(tenant))
	if err != nil {
		return nil, 0, ErrOrderFetchFailed
	}
	if err := s.ensureOrdersCanceledIfExpired(orders); err != nil {
