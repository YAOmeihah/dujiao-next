...
var resellerAccountingTestSeq uint64

func nextResellerAccountingTestID() uint64 {
	return atomic.AddUint64(&resellerAccountingTestSeq, 1)
}

func openResellerAccountingServiceTestDB(t *testing.T) *gorm.DB {
	t.Helper()
	dsn := fmt.Sprintf("file:reseller_accounting_service_%d_%d?mode=memory&cache=shared", time.Now().UnixNano(), nextResellerAccountingTestID())
...
func seedResellerAccountingProfile(t *testing.T, db *gorm.DB) models.ResellerProfile {
	t.Helper()
	user := models.User{Email: fmt.Sprintf("reseller-%d@example.test", nextResellerAccountingTestID()), PasswordHash: "x"}
...
func seedPaidResellerOrderSnapshot(t *testing.T, db *gorm.DB, eligible bool) (models.Order, models.Payment, models.ResellerOrderSnapshot) {
	t.Helper()
	user := models.User{Email: fmt.Sprintf("buyer-%d@example.test", nextResellerAccountingTestID()), PasswordHash: "x"}
	if err := db.Create(&user).Error; err != nil {
