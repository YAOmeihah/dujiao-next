...
func TestOrderServiceTenantScopedGuestQueries(t *testing.T) {
	...
	if _, err := f.svc.GetOrderByGuestOrderNoForTenant(MainTenantContext("main.example.test"), resellerOrder.OrderNo, "13800138002", "pw"); !errors.Is(err, ErrGuestOrderNotFound) {
		t.Fatalf("main scope should not read reseller guest order, got %v", err)
	}
	got, err := f.svc.GetOrderByGuestOrderNoForTenant(f.tenant, resellerOrder.OrderNo, "13800138002", "pw")
	if err != nil {
	...
	orders, total, err := f.svc.ListOrdersByGuestForTenant(f.tenant, "13800138002", "pw", 1, 20)
	if err != nil {
