...
func (s *PaymentService) ValidateChannel(channel *models.PaymentChannel) error {
	...
	// 非 official provider（epay/bepusdt/epusdt/okpay/tokenpay/vpay）只支持 qr/redirect。
	...
	if providerType != constants.PaymentProviderOfficial {
		...
		if providerType == constants.PaymentProviderVpay && mode != constants.PaymentInteractionRedirect {
			return ErrPaymentChannelConfigInvalid
		}
	}
