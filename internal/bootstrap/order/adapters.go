...
func (a orderGuestQueryAdapter) ListOrdersByGuestForTenant(tenant reseller.TenantContext, phone, password string, page, pageSize int) ([]orderdomain.Order, int64, error) {
	return a.orders.ListOrdersByGuestForTenant(tenant, phone, password, page, pageSize)
}

func (a orderGuestQueryAdapter) GetOrderByGuestOrderNoForTenant(tenant reseller.TenantContext, orderNo, phone, password string) (*orderdomain.Order, error) {
	order, err := a.orders.GetOrderByGuestOrderNoForTenant(tenant, orderNo, phone, password)
	return order, mapOrderTransportError(err)
}

func (a orderGuestQueryAdapter) GetAnyOrderByGuestOrderNoForTenant(tenant reseller.TenantContext, orderNo, phone, password string) (*orderdomain.Order, error) {
	order, err := a.orders.GetAnyOrderByGuestOrderNoForTenant(tenant, orderNo, phone, password)
	return order, mapOrderTransportError(err)
}
...
func mapOrderTransportError(err error) error {
	...
	for _, mapping := range []struct {
		...
		{orderapp.ErrGuestPhoneRequired, ordertransport.ErrGuestPhoneRequired},
		{orderapp.ErrGuestPasswordRequired, ordertransport.ErrGuestPasswordRequired},
		{orderapp.ErrInvalidPhone, ordertransport.ErrInvalidPhone},
		{orderapp.ErrInvalidEmail, ordertransport.ErrInvalidEmail},
		{orderapp.ErrShippingAddressRequired, ordertransport.ErrShippingAddressRequired},
		{orderapp.ErrShippingAddressInvalid, ordertransport.ErrShippingAddressInvalid},
