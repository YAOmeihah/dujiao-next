...
func (r *Store) Create(order *orderdomain.Order, items []orderdomain.OrderItem) error {
	if order != nil && order.UserID == 0 && strings.TrimSpace(order.GuestPassword) != "" {
		...
		order.GuestPassword = r.hashGuestCredential(order.GuestPhone, order.GuestPassword)
	}
...
func (r *Store) hashGuestCredential(phone, password string) string {
	if len(r.guestCredentialSecret) == 0 {
		panic("order store: guest credential secret is required")
	}
...
func (r *Store) BackfillGuestCredentialHashes() (int64, error) {
	...
	for {
		...
		if err := r.db.Model(&orderdomain.Order{}).
			Select("id", "guest_phone", "guest_password").
		...
		err := r.db.Transaction(func(tx *gorm.DB) error {
			for _, row := range rows {
				...
				result := tx.Model(&orderdomain.Order{}).
					...
					UpdateColumn("guest_password", r.hashGuestCredential(row.GuestPhone, row.GuestPassword))
				if result.Error != nil {
					return result.Error
				}
...
func (r *Store) GetAnyByOrderNoAndGuest(orderNo, phone, password string) (*orderdomain.Order, error) {
	var order orderdomain.Order
	query := r.withChildren(r.db)
	if err := query.Where("order_no = ? AND user_id = 0 AND guest_phone = ? AND guest_password = ?", orderNo, phone, r.hashGuestCredential(phone, password)).First(&order).Error; err != nil {
		if errors.Is(err, gorm.ErrRecordNotFound) {
			return nil, nil
		}
		return nil, err
...
func (r *Store) GetByIDAndGuest(id uint, phone, password string) (*orderdomain.Order, error) {
	var order orderdomain.Order
	query := r.withChildren(r.db)
	if err := query.
		Where("id = ? AND user_id = 0 AND guest_phone = ? AND guest_password = ? AND parent_id IS NULL", id, phone, r.hashGuestCredential(phone, password)).
...
func (r *Store) GetByOrderNoAndGuest(orderNo, phone, password string) (*orderdomain.Order, error) {
	var order orderdomain.Order
	query := r.withChildren(r.db)
	if err := query.
		Where("order_no = ? AND user_id = 0 AND guest_phone = ? AND guest_password = ? AND parent_id IS NULL", orderNo, phone, r.hashGuestCredential(phone, password)).
		First(&order).Error; err != nil {
		if errors.Is(err, gorm.ErrRecordNotFound) {
			return nil, nil
		}
...
func (r *Store) GetByIDAndGuestScoped(id uint, phone, password string, scope ordercontract.TenantScope) (*orderdomain.Order, error) {
	var order orderdomain.Order
	query := r.withChildren(r.db)
	query = applyTenantScope(query.Where("id = ? AND user_id = 0 AND guest_phone = ? AND guest_password = ? AND parent_id IS NULL", id, phone, r.hashGuestCredential(phone, password)), scope)
	if err := query.First(&order).Error; err != nil {
		if errors.Is(err, gorm.ErrRecordNotFound) {
			return nil, nil
		}
...
func (r *Store) GetByOrderNoAndGuestScoped(orderNo, phone, password string, scope ordercontract.TenantScope) (*orderdomain.Order, error) {
	var order orderdomain.Order
	query := r.withChildren(r.db)
	query = applyTenantScope(query.Where("order_no = ? AND user_id = 0 AND guest_phone = ? AND guest_password = ? AND parent_id IS NULL", orderNo, phone, r.hashGuestCredential(phone, password)), scope)
	if err := query.First(&order).Error; err != nil {
		if errors.Is(err, gorm.ErrRecordNotFound) {
			return nil, nil
		}
...
func (r *Store) GetAnyByOrderNoAndGuestScoped(orderNo, phone, password string, scope ordercontract.TenantScope) (*orderdomain.Order, error) {
	var order orderdomain.Order
	query := r.withChildren(r.db)
	query = applyTenantScope(query.Where("order_no = ? AND user_id = 0 AND guest_phone = ? AND guest_password = ?", orderNo, phone, r.hashGuestCredential(phone, password)), scope)
	if err := query.First(&order).Error; err != nil {
		if errors.Is(err, gorm.ErrRecordNotFound) {
			return nil, nil
		}
...
func (r *Store) ListByGuest(phone, password string, page, pageSize int) ([]orderdomain.Order, int64, error) {
	credentialHash := r.hashGuestCredential(phone, password)
	var total int64
	if err := r.db.Model(&orderdomain.Order{}).
		Where("orders.deleted_at IS NULL AND user_id = 0 AND guest_phone = ? AND guest_password = ? AND parent_id IS NULL", phone, credentialHash).
	...
	query := r.withChildren(r.db)
	if err := query.
		Where("user_id = 0 AND guest_phone = ? AND guest_password = ? AND parent_id IS NULL", phone, credentialHash).
		Order("id desc").
		Limit(pageSize).
...
func (r *Store) ListByGuestScoped(phone, password string, page, pageSize int, scope ordercontract.TenantScope) ([]orderdomain.Order, int64, error) {
	base := r.db.Model(&orderdomain.Order{}).Where("orders.deleted_at IS NULL AND user_id = 0 AND guest_phone = ? AND guest_password = ? AND parent_id IS NULL", phone, r.hashGuestCredential(phone, password))
