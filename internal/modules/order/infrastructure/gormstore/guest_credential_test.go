...
func TestGuestCredentialIsHashedAtRestAndRawCredentialStillQueries(t *testing.T) {
	...
	got, err := repo.GetByOrderNoAndGuest("GUEST-HASHED", "+8613800138000", "guest-password")
	if err != nil || got == nil {
		t.Fatalf("raw credential should still authenticate, got order=%v err=%v", got, err)
	}
	got, err = repo.GetByOrderNoAndGuest("GUEST-HASHED", "+8613800138000", "wrong-password")
	if err != nil {
...
func TestGuestCredentialHashLikeRawPasswordCannotBypassHashing(t *testing.T) {
	...
	got, err := repo.GetByOrderNoAndGuest(order.OrderNo, order.GuestPhone, rawPassword)
	if err != nil || got == nil {
...
func TestBackfillGuestCredentialHashesMigratesLegacyPlaintext(t *testing.T) {
	db := openOrderTenantScopeTestDB(t)
	legacy := seedScopedOrder(t, db, "GUEST-LEGACY", 0, "+8613800138002", "legacy-password", constants.OrderStatusPendingPayment, nil, nil)
	prefixedLegacy := seedScopedOrder(t, db, "GUEST-PREFIXED-LEGACY", 0, "+8613800138003", guestCredentialHashPrefix+"legacy-password", constants.OrderStatusPendingPayment, nil, nil)
	repo := New(db, "test-guest-credential-secret-with-32-bytes")
	...
	got, err := repo.GetByOrderNoAndGuest("GUEST-LEGACY", "+8613800138002", "legacy-password")
	if err != nil || got == nil {
		t.Fatalf("migrated credential should authenticate, got order=%v err=%v", got, err)
	}
	got, err = repo.GetByOrderNoAndGuest("GUEST-PREFIXED-LEGACY", "+8613800138003", guestCredentialHashPrefix+"legacy-password")
	if err != nil || got == nil {
...
func TestBackfillGuestCredentialHashesProcessesBatchesAndSkipsMigratedRows(t *testing.T) {
	...
	for i := 0; i < plaintextCount; i++ {
		rows = append(rows, orderdomain.Order{
			...
			GuestPhone:    fmt.Sprintf("138%08d", i),
	...
	migratedHash := repo.hashGuestCredential("+8613800138004", "already-hashed")
	rows = append(rows, orderdomain.Order{
		OrderNo:       "GUEST-ALREADY-HASHED",
		GuestPhone:    "+8613800138004",
...
func seedPostgresGuestOrder(t *testing.T, db *gorm.DB, orderNo, guestPhone, guestPassword string) orderdomain.Order {
	t.Helper()
	order := orderdomain.Order{
		OrderNo:       orderNo,
		UserID:        0,
...
func TestBackfillGuestCredentialHashesOnPostgres(t *testing.T) {
	...
	plaintext := seedPostgresGuestOrder(t, db, "PG-GUEST-PLAINTEXT", "+8613800138006", "legacy-password")
	prefixed := seedPostgresGuestOrder(t, db, "PG-GUEST-PREFIXED", "+8613800138007", guestCredentialHashPrefix+"legacy-password")
	// 前缀与长度都对、只有摘要体不是十六进制 —— 唯一必须走 SUBSTRING 正则分支才能捞出的形态。
	malformed := seedPostgresGuestOrder(t, db, "PG-GUEST-MALFORMED", "+8613800138008", guestCredentialHashPrefix+strings.Repeat("z", sha256.Size*2))
	alreadyHashed := repo.hashGuestCredential("+8613800138009", "already-hashed")
	untouched := seedPostgresGuestOrder(t, db, "PG-GUEST-HASHED", "+8613800138009", alreadyHashed)
	...
	got, err := repo.GetByOrderNoAndGuest("PG-GUEST-PLAINTEXT", "+8613800138006", "legacy-password")
	if err != nil || got == nil {
