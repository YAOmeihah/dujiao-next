...
func (h *Handler) ListGuestOrders(c *gin.Context) {
	...
	if orderNo != "" {
		order, err := h.OrderService.GetOrderByGuestOrderNoForTenant(tenantFromRequest(c), orderNo, phone, password)
		if err != nil {
	...
	orders, total, err := h.OrderService.ListOrdersByGuestForTenant(tenantFromRequest(c), phone, password, page, pageSize)
	if err != nil {
		shared.RespondError(c, response.CodeInternal, "error.order_fetch_failed", err)
		return
	}
...
func (h *Handler) DownloadGuestFulfillment(c *gin.Context) {
	...
	order, err := h.OrderService.GetAnyOrderByGuestOrderNoForTenant(tenantFromRequest(c), orderNo, phone, password)
	if err != nil {
		if errors.Is(err, service.ErrGuestOrderNotFound) {
			shared.RespondError(c, response.CodeNotFound, "error.guest_order_not_found", nil)
			return
...
func (h *Handler) CreateGuestPayment(c *gin.Context) {
	...
	guestOrder, err := h.OrderService.GetOrderByGuestOrderNoForTenant(tenantFromRequest(c), req.OrderNo, phone, password)
	if err != nil {
		if errors.Is(err, service.ErrGuestOrderNotFound) {
			shared.RespondError(c, response.CodeNotFound, "error.guest_order_not_found", nil)
			return
...
func (h *Handler) CaptureGuestPayment(c *gin.Context) {
	...
	if _, err := h.OrderService.GetOrderByGuestForTenant(tenantFromRequest(c), payment.OrderID, phone, password); err != nil {
		if errors.Is(err, service.ErrGuestOrderNotFound) {
			shared.RespondError(c, response.CodeNotFound, "error.guest_order_not_found", nil)
			return
		}
...
func (h *Handler) GetGuestLatestPayment(c *gin.Context) {
	...
	order, err := h.OrderService.GetOrderByGuestOrderNoForTenant(tenantFromRequest(c), query.OrderNo, phone, password)
	if err != nil {
		if errors.Is(err, service.ErrGuestOrderNotFound) {
			shared.RespondError(c, response.CodeNotFound, "error.guest_order_not_found", nil)
			return
