...
type OrderRepository interface {
	...
	GetByIDAndGuestScoped(id uint, phone, password string, scope ResellerOrderScope) (*models.Order, error)
	GetByOrderNoAndGuestScoped(orderNo, phone, password string, scope ResellerOrderScope) (*models.Order, error)
	GetAnyByOrderNoAndGuestScoped(orderNo, phone, password string, scope ResellerOrderScope) (*models.Order, error)
	...
	ListByGuestScoped(phone, password string, page, pageSize int, scope ResellerOrderScope) ([]models.Order, int64, error)
...
func (r *GormOrderRepository) GetByIDAndGuestScoped(id uint, phone, password string, scope ResellerOrderScope) (*models.Order, error) {
	var order models.Order
	query := r.withChildren(r.db.Preload("Items").Preload("Fulfillment"))
	query = applyResellerOrderScope(query.Where("id = ? AND user_id = 0 AND guest_phone = ? AND guest_password = ? AND parent_id IS NULL", id, phone, password), scope)
	if err := query.First(&order).Error; err != nil {
		if errors.Is(err, gorm.ErrRecordNotFound) {
			return nil, nil
		}
...
func (r *GormOrderRepository) GetByOrderNoAndGuestScoped(orderNo, phone, password string, scope ResellerOrderScope) (*models.Order, error) {
	var order models.Order
	query := r.withChildren(r.db.Preload("Items").Preload("Fulfillment"))
	query = applyResellerOrderScope(query.Where("order_no = ? AND user_id = 0 AND guest_phone = ? AND guest_password = ? AND parent_id IS NULL", orderNo, phone, password), scope)
	if err := query.First(&order).Error; err != nil {
		if errors.Is(err, gorm.ErrRecordNotFound) {
			return nil, nil
		}
...
func (r *GormOrderRepository) GetAnyByOrderNoAndGuestScoped(orderNo, phone, password string, scope ResellerOrderScope) (*models.Order, error) {
	var order models.Order
	query := r.db.Preload("Items").Preload("Fulfillment").Preload("Children", func(db *gorm.DB) *gorm.DB {
		return db.Preload("Items").Preload("Fulfillment")
	})
	query = applyResellerOrderScope(query.Where("order_no = ? AND user_id = 0 AND guest_phone = ? AND guest_password = ?", orderNo, phone, password), scope)
	if err := query.First(&order).Error; err != nil {
		if errors.Is(err, gorm.ErrRecordNotFound) {
			return nil, nil
		}
...
func (r *GormOrderRepository) ListByGuestScoped(phone, password string, page, pageSize int, scope ResellerOrderScope) ([]models.Order, int64, error) {
	base := r.db.Model(&models.Order{}).Where("user_id = 0 AND guest_phone = ? AND guest_password = ? AND parent_id IS NULL", phone, password)
