...
func seedScopedOrder(t *testing.T, db *gorm.DB, orderNo string, userID uint, guestPhone string, guestPassword string, status string, resellerID *uint, parentID *uint) models.Order {
	t.Helper()
	order := models.Order{
		OrderNo:          orderNo,
		ParentID:         parentID,
...
func TestOrderRepositoryTenantScopePointQueriesAndLists(t *testing.T) {
	...
	guestMain := seedScopedOrder(t, db, "GUEST-MAIN", 0, guestPhone, "code", constants.OrderStatusPendingPayment, nil, nil)
	guestReseller := seedScopedOrder(t, db, "GUEST-R1", 0, guestPhone, "code", constants.OrderStatusPaid, &resellerOne, nil)
	_ = resellerTwoOrder
	_ = child
	_ = guestMain
	_ = guestReseller
	...
	guestGot, err := repo.GetByOrderNoAndGuestScoped("GUEST-MAIN", guestPhone, "code", mainScope)
	if err != nil {
		t.Fatalf("GetByOrderNoAndGuestScoped main failed: %v", err)
	}
	assertOrderFound(t, guestGot, "GUEST-MAIN")
	guestGot, err = repo.GetByOrderNoAndGuestScoped("GUEST-MAIN", guestPhone, "code", resellerOneScope)
	if err != nil {
		t.Fatalf("GetByOrderNoAndGuestScoped main from reseller failed: %v", err)
	}
	assertOrderMissing(t, guestGot)
	guestGot, err = repo.GetByIDAndGuestScoped(guestReseller.ID, guestPhone, "code", resellerOneScope)
	if err != nil {
		t.Fatalf("GetByIDAndGuestScoped reseller failed: %v", err)
	}
	assertOrderFound(t, guestGot, "GUEST-R1")
	guestGot, err = repo.GetByIDAndGuestScoped(guestReseller.ID, guestPhone, "code", resellerTwoScope)
	if err != nil {
	...
	guestRows, guestTotal, err := repo.ListByGuestScoped(guestPhone, "code", 1, 20, resellerOneScope)
	if err != nil {
