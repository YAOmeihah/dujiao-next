...
function encodeGuestAuthorization(phone: string, orderPassword: string): string {
    const bytes = new TextEncoder().encode(`${phone.trim()}\n${orderPassword.trim()}`)
    let binary = ''
...
function splitGuestAuth(input: GuestAuthInput = {}) {
    const { phone = '', email: _email = '', order_password = '', ...payload } = input
    return {
        payload,
        headers: {
            Authorization: encodeGuestAuthorization(String(phone), String(order_password)),
        },
    }
}
