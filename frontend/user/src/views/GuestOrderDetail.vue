    ...
    <div class="container mx-auto px-4">
      ...
      <div v-if="viewState === 'auth'"
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
          <Input v-model="auth.phone" type="tel" class="h-11" :placeholder="t('guestOrders.phonePlaceholder')" />
      ...
      <div v-else-if="viewState === 'detail' && order" class="space-y-6">
        ...
        <div v-if="shippingAddressRows.length" class="rounded-2xl border bg-card shadow-sm p-6">
          <h2 class="text-lg font-bold mb-4">{{ t('orderDetail.shippingTitle') }}</h2>
          <div class="grid grid-cols-1 gap-4 text-sm md:grid-cols-2">
            <div v-for="row in shippingAddressRows" :key="row.key" class="border rounded-xl p-4">
